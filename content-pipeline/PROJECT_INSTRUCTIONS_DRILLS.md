...
> `Generate 10 drills, TOPIC=history, DIFFICULTY=medium` or (after output) `Validate`.
>
...
> type, sentence-rhythm/register rules, quote-anchored stems, and answer-position balancing.
> **Correction:** the earlier "all options within ±15% length" rule was WRONG — real CAT options
> vary ~32%; what matters is that length is *decorrelated* from correctness.
...
TOPIC ∈ {economics, history, humanities, philosophy, science, social}.  DIFFICULTY ∈ {easy, medium, tough}.
TOPIC MIX — when free to choose, follow CAT's real distribution: history/archaeology 30% ·
science/ecology 19% · society/politics/culture 16% · art/literature 11% · economics 8% ·
technology 7% · language 6% · philosophy 2%. Vary the subtopic of every paragraph in a batch.
...
── DRILLS PARAGRAPH TEXTURE (a drill paragraph is an EXCERPT, not a mini-essay) ──
Real CAT prose carries the marks of being lifted from a book or article. In 90–120 words you can't
fit them all — include ONE OR TWO per drill, varying which across the batch (measured share in full
CAT passages, for reference): named person/researcher 85% · ellipsis (…) 79% · colon 72% ·
first person 67% · em-dash aside 55% · direct quotation 51% · parenthetical aside 48% · numeric or
date detail 46% · square-bracket gloss 43% · semicolon 37% · rhetorical question 33% · italics 0%.
A batch where every paragraph is a clean, feature-free AI mini-essay will be rejected.
OPENING SHAPE — roll one per drill (measured): assertion 60% · definition-of-a-term 19% ·
narrative/scene 9% · historical framing 9% · rhetorical question 4%. Never open every drill alike.
...
Each drill has ONE question. Do NOT make them all inference. Across a batch of n items:
  ...
  • include at least one function or author_intent question when n ≥ 4,
  • at least one stem must quote the paragraph verbatim (13% of real CAT stems do this),
  • the rest spread across inference / application / assumption / vocab_in_context / main_idea,
  • tone at most one per batch (it is 1% of real CAT); bare "which title fits?" and pure
    detail/retrieval are BANNED.
...
The sentence-rhythm rule applies at every difficulty: vary sentence length, keep some under 12
words, and use plain connectives (but / yet / still), never furthermore / moreover / nevertheless.
...
• medium: TRUE EXAM LEVEL — this is the band that matters most, so calibrate it against the anchor
  questions above, not against "a bit harder than easy". The paragraph implies its point and
  contains at least one genuine turn. The correct option requires ONE real inferential step and is
  NOT a keyword-match of any sentence. The trap differs from the answer by scope, degree, or
  emphasis — it is a claim a ~90th-percentile student would actively defend, built from the
  paragraph's own words. If a distractor can be eliminated without re-reading the paragraph, the
  item is too easy for medium.
...
4. DISTRACTOR: every wrong option has a concrete reason matching its tag, none secretly correct,
   ≤1 absolute-word option per question, no archetype >30% of the batch's wrong options? else FAIL.
5. OPTION-CRAFT: COUNT IT ACROSS THE BATCH. In what share of items is the correct option the single
   longest? If above ~25% → FAIL. Is the correct option the shortest in at least a quarter of items?
   If never → FAIL. Is the answer the only hedged option anywhere → FAIL. Are distractors built from
   the PARAGRAPH's material rather than outside absolutes? else FAIL.
5b. ANSWER-POSITION: tally correctIndex across the batch. Real CAT is uniform (~25% each); the old
   Graspr bank was 47% index-0. If any index holds >35% of the batch, or index 3 is under 15% → FAIL
   and redistribute.
6. TYPE-QUOTA: does the BATCH satisfy the quota above (enough except_set/hypothetical, a
   function/author_intent item when n≥4, ≥1 verbatim-quote-anchored stem, ≤1 tone, no
   bare-title/pure-detail)? If not → FAIL and regenerate offenders.
7. REGISTER: sentence lengths genuinely varied (some under 12 words)? plain connectives, no
   furthermore/moreover/nevertheless, no tricolons, no tidy moral closing? else FAIL.
8. TEXTURE: does each paragraph carry one or two provenance marks (named figure, quotation,
   first person, date, bracketed gloss…), and do these VARY across the batch? else FAIL.
9. ECHO: compare paragraph/options to the PYQ corpus AND the anchor questions above; any close
   mirror → FAIL.
10. DIFFICULTY: does the paragraph AND question actually read at the item's "difficulty" (an "easy"
   item must not be dense/oblique; a "medium" item must meet the medium bar above — one real
   inferential step and a defensible trap; a "tough" item must not be trivially paraphrased)? else FAIL.
...
Spread correctIndex evenly across the batch, and never default to 0.
"type" MUST be one of: except_set, inference, function, assumption, application, vocab_in_context,
main_idea, hypothetical, tone, detail. Every item MUST carry a "difficulty" of easy|medium|tough
matching the command. Output JSON only.
```
