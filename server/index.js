  ...
  app.get("/api/dashboard/trend", authenticate, async (req, res, next) => {
    try {
      ...
      const rows = await db.all(
        // Day as a 'YYYY-MM-DD' STRING, not a DATE - same reason as the heatmap
        // below. A date column comes back as a JS Date at local midnight, which
        // serializes to the previous day in UTC ('2026-07-12' ->
        // '2026-07-11T18:30:00Z' in IST), so the chart's x-axis labels would
        // read a day early. Bucketing is unchanged: to_char reads the
